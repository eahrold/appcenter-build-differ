...
class DiffPrinter {
  ...
  print() {
    ...
    if (this.changed.length) {
      console.log(`--------- What Was Changed in ${this.type} ---------------------------------------------`);
    ...
    if (this.missing.length) {
      console.log(`--------- What Was Removed from ${this.type} ---------------------------------------------`);
...
class NpmDiffProcessor {
  ...
  _filterReduce(lines) {
    const delim = "├─";
    return lines.filter(l => l.includes(delim)).reduce((map, line) => {
...
const downloadLogs = (buildNumber, app) => {
  return new Promise((resolve, reject) => {
    ...
    function onClose(code) {
      if (code !== 0) {
        return reject(new Error(`Unable to download logs for build ${buildNumber} of ${app}\n${[...stdout, ...stderr].join("\n")}`));
      }

      return resolve(stdout);
    }

    const ls = spawn(APP_CENTER_CLI, ["build", "logs", `--id=${buildNumber}`, `--app=${app}`]);
...
const parseArgs = () => {
  ...
  argv.forEach(arg => {
    if (arg.includes("--")) {
      ...
      opts[key.replace("--", "").trim()] = value === undefined ? true : value;
    }
  });
  return {
    args,
